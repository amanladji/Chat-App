...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10 bg-gradient-to-b from-[#3c334b] to-[#2a2336] text-zinc-100 relative">
      <Link to="/settings" className="absolute top-4 right-4 z-50 text-xs font-medium bg-white/10 hover:bg-white/15 text-white/80 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/15 transition-colors flex items-center gap-2">
        <span>Settings</span>
      </Link>
        ...
        <div className="md:w-1/2 w-full p-8 sm:p-12 bg-[#1f1a27]/95 flex flex-col justify-center">
          <div className="max-w-md w-full mx-auto">
            ...
            <p className="text-sm text-zinc-400 mb-8">
              ...
              <Link to="/signup" className="text-violet-400 hover:text-violet-300 font-medium underline-offset-4 hover:underline">Create one</Link>
            </p>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full h-12 rounded-xl bg-[#2a2434] border border-[#3b3346] focus:border-violet-400 focus:ring-2 focus:ring-violet-400/40 outline-none pl-12 pr-4 text-sm placeholder:text-zinc-500 transition-colors ${errors.email ? 'border-rose-400 focus:ring-rose-400/40 focus:border-rose-400' : ''}`}
                    placeholder="you@example.com"
                    value={formData.email}
                    onChange={(e)=>setFormData({...formData,email:e.target.value})}
                  />
                </div>
                {errors.email && <p id="email-error" className="mt-2 text-xs text-rose-400 font-medium">{errors.email}</p>}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full h-12 rounded-xl bg-[#2a2434] border border-[#3b3346] focus:border-violet-400 focus:ring-2 focus:ring-violet-400/40 outline-none pl-12 pr-12 text-sm placeholder:text-zinc-500 transition-colors ${errors.password ? 'border-rose-400 focus:ring-rose-400/40 focus:border-rose-400' : ''}`}
                    placeholder="••••••••"
                ...
                {errors.password && <p id="password-error" className="mt-2 text-xs text-rose-400 font-medium">{errors.password}</p>}
                <div className="flex items-center justify-between mt-3">
                  ...
                  <button type="button" className="text-xs font-medium text-violet-400 hover:text-violet-300 underline-offset-4 hover:underline">Forgot password?</button>
                </div>
              </div>
